...
async function main(){
 ...
 if(!['ios','android'].includes(options.platform))throw new Error('Choose --platform ios or android.');
 ...
 const shot=async()=>{const name='screenshot-'+(report.artifacts.length+1)+'.png';await D.screenshot(device,path.join(dir,name));report.artifacts.push({name,kind:'image',ms:Date.now()-report.startedAt});append('note','Screenshot added: '+name);persist();console.log('Screenshot saved. Review every pixel before sharing.');};
