...
> **Early prototype · v0.1.0.** Load it unpacked in Chrome. There is no Chrome Web Store listing yet. BugDrop captures debugging evidence; it does not automatically fix bugs or generate regression tests.
...
5. Reproduce the bug. Optionally add a screenshot from the extension popup.
6. Click **Stop & review**. Add expected and actual behavior, remove any private evidence, and confirm review.
7. **Copy for coding agent**, **Download Markdown**, or **Download JSON**.
...
- Clicks on interactive elements and form-change markers, without input values.
...
- SPA route changes, relative event timing, viewport, and browser information.
- One optional screenshot of the visible page, explicitly requested by the reporter.
- Editable context, removable events, and a portable JSON report.

Markdown includes agent guidance to treat the captured content as **untrusted evidence**, verify causes, and avoid claiming tests were run when they were not. A screenshot is included in JSON; when sharing Markdown, download and attach the screenshot separately.
...
- Full reload/navigation ends capture. Start another recording on the new document. SPA route changes stay in the current capture.
...
Runtime and unit tests have no third-party dependencies. Packaging uses `zip` (macOS/Linux) and writes `dist/bugdrop-0.1.0.zip`. See [CONTRIBUTING.md](CONTRIBUTING.md) for the real-browser test and architecture.

## Next, driven by actual bug reports

- Better capture across page navigation with an explicit permission design.
- Screenshot masking controls.
